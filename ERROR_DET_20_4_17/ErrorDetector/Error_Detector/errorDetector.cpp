...
void applicationFunc()
{
	...
	GameLogic* game = new GameLogic(true, g_testParams.rectPatternId+3);
	...
	while (TERMINATION_FLAG == false)
	{
		...
		movingFactorX = elapsed.asSeconds()*RECT_MOVING_FACTOR_X *speedFactor;
		movingFactorY = elapsed.asSeconds()*RECT_MOVING_FACTOR_Y *speedFactor;
		...
		patternCounter++;
		if(patternCounter >= game->getFrameNum()){
			patternCounter = 0;
			game->updatePattern();
			//movingFactorX *= (game->_pattern.getNextAngleX());
			game->changeXDirection *= game->_pattern.getXDir();
			game->changeYDirection *= game->_pattern.getYDir();
			//movingFactorY *= (game->_pattern.getNextAngleY());
		}	
		movingFactorY *= (game->_pattern.getNextAngleY());
		movingFactorX *= (game->_pattern.getNextAngleX());
